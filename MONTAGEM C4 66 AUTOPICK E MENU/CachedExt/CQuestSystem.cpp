...
{
	CUser test;
	CQuestSystem::MoveQuestData((__int32)&test.quests - (__int32)&test,
		sizeof(test.quests) / sizeof(QuestData),
		(INT32)&CQuestSystem::FixSaveMemo);
}


void CQuestSystem::MoveQuestData(__int32 index, __int8 maxQuests, INT32 newSaveMemo)
{	
// 0x49E840 CUser::LoadQuestFromDB, 0x525174		EXEC lin_LoadQuest %d
	// .text:0049E855                 sub     esp, 418h
	WriteMemoryDWORD(0x49E857, 0x418 + 0xA0);
	// .text:0049E9E8                 add     esp, 424h
	WriteMemoryDWORD(0x49E9EA, 0x424 + 0xA0);
	// .text:0049E8D3                 lea     esi, [ebx+434h]
	WriteMemoryDWORD(0x49E8D5, index + 4);
	// .text:0049E914                 mov     edi, 10h
	WriteMemoryDWORD(0x49E915, maxQuests);

// 524F20		EXEC lin_SetQuest
	WriteInstruction(0x49E607, newSaveMemo, 0, 0xE9);
	// .text:0049E7EA                 add     esp, 10Ch
	WriteMemoryDWORD(0x49E7EC, (maxQuests*4+3)*4);
	// .text:0049E7DD                 lea     eax, [esp+538h+var_420]
	WriteMemoryDWORD(0x49E7E0, (maxQuests*4+3)*4 + 0xC);
// 0x4AEC70 CUser::CUser
	// .text:004AEEDE                 lea     eax, [ebx+434h]
	WriteMemoryDWORD(0x4AEEE0, index + 4);
	// .text:004AEEE4                 mov     esi, 10h
	WriteMemoryDWORD(0x4AEEE5, maxQuests);

// CUser::SetSelectedQuest
	// .text:004A2482                 lea     edx, [esi+430h]
	WriteMemoryDWORD(0x4A2484, index);
	// .text:004A249A                 cmp     eax, 10h
	WriteMemoryBYTE(0x4A249C, maxQuests);
	// .text:004A24AC                 cmp     eax, 10h
	WriteMemoryBYTE(0x4A24AE, maxQuests);
	// .text:004A24C5                 mov     [eax+esi+434h], ecx
	WriteMemoryDWORD(0x4A24C8, index + 4);
	// .text:004A24B3                 lea     edx, [eax+43h]
	WriteMemoryBYTE(0x4A24B5, index/16);

//  CUser::DeleteSelectedQuest
	// .text:004A2593                 lea     ecx, [esi+430h]
	WriteMemoryDWORD(0x4A2595, index);
	// .text:004A25AA                 cmp     eax, 10h
	WriteMemoryBYTE(0x4A25AC, maxQuests);
	// .text:004A25BD                 mov     [eax+esi+434h], ebx
	WriteMemoryDWORD(0x4A25C0, index + 4);
	// .text:004A25B1                 lea     ecx, [eax+43h]
	WriteMemoryBYTE(0x4A25B3, index/16);

// 0x4A8650 CUser::LoadQuest
	// .text:004A8756                 lea     esi, [ebx+438h]
	WriteMemoryDWORD(0x4A8758, index + 8);
	// .text:004A875C                 mov     ebp, 10h
	WriteMemoryDWORD(0x4A875D, maxQuests);

// 0x4A8300 CUser::SetQuest
	// .text:004A83AC                 lea     edi, [ebp+43Ch]
	WriteMemoryDWORD(0x4A83AE, index + 0x0C);
	// .text:004A84A4                 cmp     eax, 10h
	WriteMemoryBYTE(0x4A84A6, maxQuests);
}


__declspec(naked) void CQuestSystem::FixSaveMemo()
{
	__asm
	{
		mov     eax, [esi+0C4h]
